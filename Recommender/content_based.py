...
def get_text_based_recommendations(listing_id, df_grouped, textual_embeddings, num_recommendations=10):
    ...
    recommendations = df_grouped.iloc[similar_indices].copy()

    recommendations['text_similarity_score'] = [sim_scores[i][1] for i in range(1, num_recommendations + 1)]
...
def refine_with_structured_features(text_based_recommendations, structured_features, original_index,
    ...
    sim_scores_structured = cosine_similarity([structured_features[original_index]], text_based_structured_features).flatten()
    ...
    # Include structured similarity scores in the recommendations DataFrame
    refined_recommendations = text_based_recommendations.iloc[top_structured_indices].copy()

    refined_recommendations['structured_similarity_score'] = sim_scores_structured[top_structured_indices]

    # Calculate final similarity score as a weighted average of text and structured similarity scores
    refined_recommendations['final_similarity_score'] = (
        refined_recommendations['text_similarity_score'] *0.5 + refined_recommendations['structured_similarity_score'] * 0.5
    )
